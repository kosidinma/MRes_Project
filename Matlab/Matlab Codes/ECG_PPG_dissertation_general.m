...
for i = 6: 1 : m - 6
    if ((A3(i) >= A3(i+1)) &&(A3(i) >= A3(i+2)) &&(A3(i) > A3(i+3)) &&(A3(i) > A3(i+4)) && (A3(i) >= A3(i-2))&&(A3(i) >= A3(i-1)) &&(A3(i) > A3(i-3)) &&(A3(i) > A3(i-4))) 
...
ave_peak_dif = round((jy/pk_place))-1;
...
for g = 2 : 1 : pk_place-1 
    ...
    Time(window_no+1) = Timee(window_no+1) + Data_start+ave_peak_dif - 1; %correct time to cover all data
    Peak_Time(window_no+1) = Sampling_rate * Time(window_no+1); %correct time with sampling rate

    %%% PPG ANKLE PROCESSING
    [Peak2(window_no+1),Time2(window_no+1)] = min(A2(Data_start2:Data_start2 + Timee(window_no+1))); %get peak and corresponding time value in window
    ...
    [Peak3(window_no+1),Time3(window_no+1)] = min(A3(Data_start:Data_starts + Timee(window_no+1))); %get peak and corresponding time value in window
...
if (mean(PAT_ankle) < 0)
    for i = 2: 1 : window_no
        PAT_ankle(i) = abs(Peak_Time(i) - Peak_Time2(i-1));
    end
else
    for i = 2: 1 : window_no
        PAT_ankle(i) = abs(Peak_Time(i) - Peak_Time2(i));
    end
end

if (mean(PAT_finger) < 0)
    for i = 2: 1 : window_no
        PAT_finger(i) = abs(Peak_Time(i) - Peak_Time3(i-1));
    end
else
    for i = 1: 1 : window_no
        PAT_finger(i) = abs(Peak_Time(i) - Peak_Time3(i));
    end
end
