	...
	public View onCreateView(final LayoutInflater inflater, final ViewGroup container, final Bundle savedInstanceState) {
		...
		try {
			if (c.moveToNext()) {
				...
				bpm = battery;
				if (battery == 0){
	            	hr.setBackgroundResource(R.drawable.noheartbeat);
	            }
	            else {
	            	hr.setBackgroundResource(R.drawable.heartpicsmall);
					batteryView.setText(getString(R.string.battery, battery));
	            }
				//batteryView.getBackground().setLevel(battery);
				//batteryView.setBackground(null);
				//heartbeat.run();
			}
		} finally {
			c.close();
...
//		graphView.setVisibility(View.GONE);
	...
	public void onCreateOptionsMenu(final Menu menu, final MenuInflater inflater) {
		...
		final boolean visible = true; //added by me
...
//			break;
	...
	private Runnable heartbeat = new Runnable() { //removed for speed
		@Override
		public void run() {
