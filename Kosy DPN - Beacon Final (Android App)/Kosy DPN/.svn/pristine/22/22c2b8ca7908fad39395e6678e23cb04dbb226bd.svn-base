 ...
 * 
 * GraphView is free software: you can redistribute it and/or modify it under the terms of the GNU Lesser General Public License as published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * 
 * GraphView is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General
 * Public License for more details.
 * 
 * You should have received a copy of the GNU General Public License along with GraphView. If not, see <http://www.gnu.org/licenses/lgpl.html>.
 * 
 ...
 * {@link #drawSeries(Canvas, GraphViewDataInterface[], float, float, float, double, double, double, double, float)} to display a custom graph. Use {@link LineGraphView} for creating a line chart.
 * 
...
abstract public class GraphView extends LinearLayout {
	...
	private class GraphViewContentView extends View {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (labelTextHeight == null || horLabelTextWidth == null) {
				...
				String testLabel = formatLabel(0, 1, testX, true);
	...
	private class VerLabelsView extends View {
		...
		public VerLabelsView(Context context) {
			super(context);
			setLayoutParams(new LayoutParams(getGraphViewStyle().getVerticalLabelsWidth() == 0 ? 100 : getGraphViewStyle().getVerticalLabelsWidth(), LayoutParams.FILL_PARENT));
		}
		...
		protected void onDraw(Canvas canvas) {
			...
			if (labelTextHeight == null || verLabelTextWidth == null) {
				...
				String testLabel = formatLabel(0, 1, testY, false);
				paint.getTextBounds(testLabel, 0, testLabel.length(), textBounds);
				labelTextHeight = (textBounds.height());
	...
	abstract protected void drawSeries(Canvas canvas, GraphViewDataInterface[] values, float graphwidth, float graphheight, float border, double minX, double minY, double diffX, double diffY,
	 ...
	 */
	@Deprecated
	protected String formatLabel(int index, int total, double value, boolean isValueX) {
		if (customLabelFormatter != null) {
			String label = customLabelFormatter.formatLabel(index, total, value, isValueX);
			if (label != null) {
				return label;
			}
		}
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		for (int i = 0; i <= numLabels; i++) {
			labels[i] = formatLabel(i, numLabels, min + ((max - min) * i / numLabels), true);
		}
		return labels;
	}
	...
	synchronized private String[] generateVerlabels(float graphheight) {
		...
		for (int i = 0; i <= numLabels; i++) {
			labels[numLabels - i] = formatLabel(i, numLabels, min + ((max - min) * i / numLabels), false);
		}
		return labels;
	}
	...
	/**
	 * you have to set the bounds {@link #setManualYAxisBounds(double, double)}. That automatically enables manualYAxis-flag. if you want to disable the menual y axis, call this method with false.
	 * 
	 * @param manualYAxis
	 */
	 ...
	 *            x-value
	 * @param size
	 *            the view port size. If 0 than all data will be shown
	 */
